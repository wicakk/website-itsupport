...
const CATEGORIES = ['Web Development','Mobile App','Infrastructure','Security','Network','Database','UI/UX','Lainnya']
...
const ModalShell = ({ onClose, children, maxWidth=520, theme }) => (
...
function ProjectFormModal({ project, users, onClose, onSave, loading, theme }) {
  ...
  const [form, setForm] = useState({
    ...
    category: project?.category??'', priority: project?.priority??'medium', color: project?.color??'#6366f1',
    status: project?.status??'active', due_date: project?.due_date??'',
  ...
  const handleFiles = e => {
    ...
    setFiles(prev => {
      ...
      const newFiles = picked.filter(f=>!existing.includes(f.name))
    ...
    <ModalShell onClose={onClose} theme={theme}>
      ...
      <div style={{ padding:20, display:'flex', flexDirection:'column', gap:14, overflowY:'auto' }}>
        ...
        <div>
          ...
          <textarea value={form.description} onChange={set('description')} rows={4} placeholder="Deskripsi project..." style={{ ...inp(theme), resize:'vertical' }}/>
        </div>
        {/* Warna */}
        <div>
      ...
      <div style={{ display:'flex', justifyContent:'flex-end', gap:8, padding:'12px 20px', borderTop:`1px solid ${theme.border}`, flexShrink:0 }}>
        ...
        <button onClick={()=>{ if(!form.name.trim()){ setErr({name:'Wajib diisi'}); return } onSave(form) }} disabled={loading}
...
function ProjectCard({ project, onEdit, onDelete, canManage, theme, onClick }) {
  ...
  const progress = project.task_stats?.total > 0 ? Math.round(((project.task_stats?.completed??0)/project.task_stats.total)*100) : 0
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end' }}>
          ...
          <div>
            <div style={{ display:'flex', marginBottom:3 }}>
              ...
              {(project.members??[]).length > 4 && (
                <div style={{ width:22, height:22, borderRadius:'50%', background:theme.surfaceAlt, border:`2px solid ${theme.surface}`, display:'flex', alignItems:'center', justifyContent:'center', fontSize:8, color:theme.textMuted, marginLeft:-5 }}>+{(project.members??[]).length-4}</div>
              )}
            </div>
          ...
          <div style={{ textAlign:'right' }}>
            ...
            <div style={{ fontSize:9, color:theme.textMuted, marginTop:1 }}>
              {fmtDateTime(project.created_at)}
            </div>
          </div>
        </div>
      </div>
...
export default function ProjectsPage() {
  ...
  const handleSave = async (form, files=[]) => {
    ...
    try {
      ...
      // Upload file lampiran ke project
      ...
      if (files.length > 0 && projectId) {
        ...
        for (const file of files) {
          try {
            ...
            else console.warn('Upload gagal:', json.message)
          } catch(e) { console.warn('Upload file error:', e) }
        }
        if (uploadedCount > 0) showToast(`${uploadedCount} file berhasil diupload ✓`)
      }

      setFormModal({ open:false, project:null })
      // Refresh list project agar attachment count terupdate
      fetchProjects()
    ...
    <div style={{ display:'flex', flexDirection:'column', gap:20 }}>
      ...
      {loading ? (
        ...
        <div style={{ textAlign:'center', padding:'60px 20px' }}>
          <FolderKanban size={40} color={theme.textMuted} style={{ margin:'0 auto 12px' }}/>
          <div style={{ fontSize:14, fontWeight:600, color:theme.text }}>Belum ada project</div>
          <div style={{ fontSize:12, color:theme.textMuted }}>{isManagerIT?'Klik "Buat Project" untuk memulai.':'Anda belum ditambahkan ke project manapun.'}</div>
        </div>
      ) : (
        <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(280px,1fr))', gap:16 }}>
          {filtered.map(p=>(
            <ProjectCard key={p.id} project={p} canManage={isManagerIT} theme={theme}
              ...
              onEdit={async(proj)=>{
                try { const r=await fetch(`/api/projects/${proj.id}`,{ headers:{ Accept:'application/json', Authorization:`Bearer ${localStorage.getItem('token')}` } }); const j=await r.json(); setFormModal({ open:true, project:j.success?j.data:proj }) }
      ...
      {formModal.open && <ProjectFormModal project={formModal.project} users={users} onClose={()=>!actionLoading&&setFormModal({ open:false, project:null })} onSave={handleSave} loading={actionLoading} theme={theme}/>}
      ...
      {deleteModal && (
        <div onClick={()=>setDeleteModal(null)} style={{ position:'fixed', inset:0, background:theme.overlay, backdropFilter:'blur(4px)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:50 }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:theme.surface, border:`1px solid ${theme.border}`, borderRadius:14, padding:24, width:360, textAlign:'center', boxShadow:'0 25px 60px rgba(0,0,0,0.3)' }}>
            <Trash2 size={28} color={theme.danger} style={{ margin:'0 auto 12px' }}/>
