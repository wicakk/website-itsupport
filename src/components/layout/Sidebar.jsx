...
const ICONS = {
  LayoutDashboard, Ticket, Package, BookOpen,
  Activity, BarChart3, Users, Settings, Shield,
}
...
const Sidebar = ({ navItems }) => {
  ...
  // Gunakan NAV_ITEMS internal — roles sudah ada di sini
  // Abaikan navItems dari props supaya tidak ada ketergantungan eksternal
  ...
  return (
    <aside style={{
      ...
      {/* Logo */}
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '0 16px', minHeight: 60, borderBottom: `1px solid ${T.border}`, transition: 'border-color 0.3s ease' }}>
      ...
      {/* Navigation */}
      <nav style={{ flex: 1, padding: '12px 8px', display: 'flex', flexDirection: 'column', gap: 2, overflowY: 'auto', scrollbarWidth: 'thin', scrollbarColor: `${T.scrollbar} transparent` }}>
        {items
          .filter(item => {
      ...
      {/* Footer */}
      <div style={{ padding: 8, borderTop: `1px solid ${T.border}`, transition: 'border-color 0.3s ease' }}>
        <button
          onClick={() => setSidebarCollapsed(!sidebarCollapsed)}
          style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', fontSize: 11, color: T.textMuted, background: isDark ? 'rgba(255,255,255,0.04)' : 'rgba(0,0,0,0.04)', border: `1px solid ${T.border}`, borderRadius: 8, cursor: 'pointer', transition: 'background 0.2s, color 0.2s', justifyContent: sidebarCollapsed ? 'center' : 'flex-start' }}
        ...
        <button
          onClick={logout}
          style={{ width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '8px 12px', fontSize: 11, color: T.textMuted, background: 'none', border: 'none', borderRadius: 8, cursor: 'pointer', transition: 'color 0.2s', justifyContent: sidebarCollapsed ? 'center' : 'flex-start' }}
