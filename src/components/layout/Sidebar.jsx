...
const NavItem = ({ item, active, collapsed, onClick }) => {
  ...
  return (
    <button
      ...
      onMouseLeave={e => {
        e.currentTarget.style.background = active ? `${T.accent}18` : 'transparent'
        e.currentTarget.style.color = active ? T.accent : T.textMuted
      }}
      style={{
        width: '100%',
        display: 'flex',
...
const Sidebar = ({ navItems }) => {
  ...
  return (
    ...
    >
      ...
      <div
        style={{
          padding: sidebarCollapsed ? '18px 14px' : '18px 16px',
      ...
      >
        ...
        {!sidebarCollapsed && (
          <div style={{ overflow: 'hidden' }}>
            <div style={{ color: T.text, fontSize: 13, fontWeight: 800, whiteSpace: 'nowrap' }}>
              IT Support
            </div>
            <div style={{ color: T.textDim, fontSize: 9, marginTop: 1, whiteSpace: 'nowrap' }}>
              Management System
            </div>
          </div>
        )}
